...
Open Animate provides 14 transition presets for use with `@remotion/transitions` `TransitionSeries`. Each returns a `TransitionPresentation` that you pass to `<TransitionSeries.Transition>`.

...
description: Hosted services for oanim — media gateway, cloud rendering, and more
---

oanim is an **open-core platform** — everything is open source (Apache 2.0), and you can self-host it all for free. The hosted platform at `api.oanim.dev` provides managed infrastructure so you don't have to.
...
<CardGroup cols={2}>
  ...
  <Card title="Cloud Rendering" icon="cloud">
    `oanim render --cloud` sends your composition to hosted infrastructure and streams back the rendered MP4. No local Chromium or ffmpeg needed.
  </Card>
  <Card title="Authentication" icon="lock">
    Browser-based OAuth via `oanim login`. API keys for CI/CD and programmatic access.
  </Card>
  <Card title="Usage & Billing" icon="chart-bar">
    Pay-as-you-go credits with per-operation cost tracking. Purchase credits via Stripe with a 10% bonus on purchases over $50.
  </Card>
</CardGroup>
...
1. Explicit `ANIMATE_API_KEY` environment variable
